...
\begin{document}
	...
	\begin{proof}
		...
		Furthermore, one can prove the map is bijective and that the inverse is smooth.
	\end{proof}
	...
	\begin{proof}
		...
		$$d_\mu{(p,g)}(X_p,dl_gA)=dr_g(X_p)+\overline{A}_{p\cdot g}$$
	\end{proof}
	\begin{Theo}
		...
		Where $g_{ji}$ is the transition function between the local trivializations $s_i,s_j$ and $\mu_{ij}=g_{ij}^*\theta$ with $\theta$ the Maurer-Cartan form.
	\end{Theo}
	\begin{proof}
		By construction, $A_i=s_i^*A$ so that for any vector field $X\in \Gamma(P)$, we have:
		$$s^*A(X)=A(ds(X))$$
		Let $p\in P$ and $x\in M$
	\end{proof}

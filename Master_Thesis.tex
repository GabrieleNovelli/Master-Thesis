...
\begin{document}
	...
	\end{Obs}
	\begin{Ex}
		The space $\mathbb{R}^n$ is orientable and an orientation is given by the global frame $\{\partial\over \partial_{r^i}\}$ where $r^i$ are the standard coordinates.
	\end{Ex}
	\begin{Ex}
	...
	In this section we will introduce the notion of exterior derivative and hodge operator. Furthermore, we will briefly look at integration on manifolds. More informations on those topic can be found in [3] chap. 7 pag 401-413.
	\begin{Def}
	...
	\begin{Obs}
		...
		In particular, at any $p\in M$, we have a smooth inner product of vectors. Since $g$ is a bilinear, symmetric and non degenerate, it induces an isomotphism on the cotangent space at any point. The inner prduct of 1-forms is defined as being equal to the inner product of the corresponding vectors. More on this construction will be said in section \ref{Sec_3.3}.
		The extension of this product to $k$-forms is clearly symmetric. The bilinearity follows from the multilinearity of the determinant for sums of rows and columns. As for the non degeneracy, it is a consequence of the non degeneracy of $\braket{,}$. Lastly, since $\braket{,}$ is smooth, this induced product also is.
	\end{Obs}
	\begin{Def}
	...
	\begin{proof}
		...
		As for the third one, consider the following: we can always expand $\star dx^I_p=\sum_Jc_p^{IJ}dx_p^{J'}$ since it is a $n-k$ form. The sum is on every ordered subset $J$ of ${1,...,n}$ of length $k$. Taking now the wedge product from both sides with $\sum_J dx^J$ we get:
		...
		As for the last property, we can prove it for an orthonormal base $\{de_p^i\}$. By construction, if $I$ is a subset of length $k$ of $\{1,2,...,n\}$, $\star de_p^I=[de_p^i]\braket{de_p^I,de_p^I}[IJ'] de_p^{J'}$ where $J'$ is the ordered complement of $I$. In fact, being an orthonormal base, the only non 0 term in the above sum is the one where $I=J$. Consider then the following equations:
		...
		By substituting the previous expression for $\star de^I$, we see that $\braket{\star de_p^I,\star de_p^I}=\braket{de_p^{J'}, de_p^{J'}}$. Also, by the alternating property of the exterior algebra, by exchanging the forms we get: $\star de_p^I\wedge (\star^2 de_p^I)=\star^2 de_p^I\wedge (\star de_p^I)\cdot (-)^{k(n-k)}$. Clearly, combining the two previous expressions, one finds:
		$$\star^2 de^I=(-)^{k(n-k)}\braket{de_p^{J'}, de_p^{J'}}\braket{de_p^{I}, de_p^{I}}de^I$$
		And since we have an orthonormal base, we have $\braket{de_p^{J'}, de_p^{J'}}\braket{de_p^{I}, de_p^{I}}=sgn(g)$. This proves the last property.
	\end{proof}
	...
	\section{The tangent-cotangent isomorphism}\label{Sec_3.3}
		...
		Selecting the canonical base $\{e_i\}$ for $V$, the map $g$ will be a matrix with entries $g_{ij}$ and its action on the components of $v\in V$ will be given by the standard rows-columns product:
		$$v\rightarrow
	...
	\begin{proof}
		...
		This map is well defined as it does not depend neither on the fiber point $p\in P$, nor on the lift. To see this, suppose to take any other set lifter vectors $\{u'_i\}$ such that $d\pi(u'_i)=v_i$. Then we must have that $u_i-u_i'$ are  all vertical due to
		$$d\pi(u_i-u'_i)=0$$
		However, by definition, $\omega$ is horizontal and multilinear, so that:
		$$\omega_p(u_1+vert.,...,u_k+vert.)=\omega_p(u_1,...,u_k)$$
		it does not depend on the choice of the lift. To see that this map does not depend on the point of the fiber, choose any other point $q\in P_x$. Then, there will exist a $g\in G$ such that $p\cdot g=q$. Then, since $\pi\circ dr=\pi$, the lifted vectors of $v_i$ will be $dr_g(u_i)$. Evaluating:
	...
	In this chapter we will introduce the concept of gauge transformations. We will study a particular family of maps on a given principal associated bundle and see how they extend on its associated bundle. Furthermore, we will see how the connection 1-forms and the associated curvatures will behave under such transformations. More informations on those topics can be found in [3] chap. 5.
	...
	In this section we compute how the connection $1$-forms and the curvature change under a change of local gauge. More information on this topic can be found in: REFERENZA
	\begin{Obs}
		...
		Clearly, given a base for the Lie algebra $\{T^a\}$, we can expand:
	...
	This base induces the following relations:
	\begin{multicols}{2}
		\begin{itemize}
	...
	$$F_{\mu\nu}=\partial_\mu A_\nu-\partial_\nu A_\mu$$
	Looking at the Maxwell's equations, $dF=0$ is automatically satisfied. As for $d^*F=0$, we can explicitly compute this in the orthonormal base:
	...
	$$\braket{dx^\mu,dx^\mu}\partial_\nu (\partial_\mu A_\nu-\partial_\nu A_\mu)=0$$
	SPAZIO CURVO???
	Lastly, we look at the lagrangian of the maxwell equations. In particular, we wish to construct a Lorentz-invariant lagrangian from which we can recover our starting equations.
	\begin{Prop}
	...
	\end{Prop}
	\begin{proof}
		We explicitly compute in coordinates the lagrangian, by substituting the previous expansions for $F,\star F$ REFERENZA:
	...
	Thus, having chosen a base for $\mathfrak{g}$, indexed with $\{T^a\}$, and a chart $(U,\phi)$ on $\mathbb{R}^{1,3}$ we can express the form as a linear combination:
	$$A_{\mathbb{R}^{1,3}}=A_{a,\mu} dx^\mu\otimes T^a=A_a\otimes T^a$$
	Here $a$ indicates the Lie algebra indices and $\mu$ the spatial indices. In the electromagnetic setting, we have only one vector potential. For this reason, to recover the Maxwell description, we set $\mathfrak{g}=\mathbb{R}$. 
	\begin{Obs}
	...
	\end{Obs}
	\textbf{The only compact Lie group with Lie algebra $\mathbb{R}$ is $U(1)$????}. Thus, we assume our principal bundle to be a $U(1)$ principal bundle. 
	\begin{Obs}
		With this choice, we also recover the gauge symmetry of the vector potential. To see this, consider two local gauges on the same open set $s_{1,2}:U\rightarrow P$, then, by theorem \ref{Theo_7.2.1}, the pulled back 1 forms are related by:
	...
	\end{Obs}
	We now take a look at the curvature. It is known that, given a connection on a principal bundle, we can construct it's curvature as an $Ad$-tensorial $2$ form:
	$$F=dA+{1\over 2}[A,A]\in \Omega^2_{Ad}(P,\mathfrak{g})$$
	This object transforms under our gauge symmetry as found in observation \ref{Obs_7.2.2}:
	$$F\rightarrow Ad(g_{ji}^{-1})F$$
	Once again, in our current setting the symmetry group is abelian and so the adjoint representation is trivial:
	$$F\rightarrow F\hbox{ is invariant under gauge transformations}$$
	From proposition \ref{Prop_7.2.2}, we know that this form is globally defined on $M$, which makes it globally closed.\\
	\\
	We now try to recover the electromagnetic lagrangian as the lagrangian of a $U(1)$ Yang-Mills theory.
	\begin{Obs}
		The curvature is a type $Ad$ tensorial form $F\in \Omega^2_{Ad}(P,\mathfrak{g})$. In proposition \ref{Mus_Iso} we found an isomorphism: $$\Omega^k_\rho(P,V)\simeq \Omega^k(M,E)$$
		Thus, the form $F$ defines a unique form $F_M$ on the manifold, with values on the adjoint associated bundle. \\
		Moreover, the induced form on the associated bundle is closed as well since:

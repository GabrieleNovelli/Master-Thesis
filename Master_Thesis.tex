...
\begin{document}
	...
	\begin{Def}
		Let $\omega=f(x)dx^1\wedge...\wedge dx^n\in\Omega^n_c(\mathbb{R}^n)$. We defines the integral of $\omega$ on a subset $U\subset\mathbb{R}^n$ as:
	...
	\begin{Obs}\label{Ex_3.4.2}
		...
		$$(r_g^*\theta)_{h}(X_{h})=\theta_{gh}(dr_gX_h)=\ell_{(gh)^{-1}}dr_gX_h=Ad(g^{-1})d\ell_{h^{-1}}X_h$$
	\end{Obs}
	\begin{Prop}
	...
	\begin{proof}
		...
		$$Ad(g)=dc_g:\mathfrak{g}\rightarrow\mathfrak{g}$$
	...
	In this section we compute how the connection $1$-forms and the curvature transform under a change of local gauge. More information on this topic can be found in: [3] chap. 5 pag. 270-285.
	\begin{Obs}
	...
	\begin{proof}
		...
		$$d\mu_{(p,g)}(X_p,d\ell_gA)f=(X_p,\ell_gA)(f\circ \mu(p,g))=(X_p(f\circ r_g(p)),\ell_gA(f\circ \ell_p(g)))$$
		where we have indicated with $\ell_p(g)=p\cdot g$ the left translation with $p$. Clearly, the first term can be rewritten like: $dr_gX_p(f)$. As for the second term instead, by definition of differential:
		$$d\ell_gA(f\circ \ell_p(g))=d\ell_p\circ d\ell_gA(f)$$
		However, $d\ell_p\circ d\ell_g=dj_{pg}$ and so we get the result:
		$$d\mu_{(p,g)}(X_p,\ell_gA)=dr_g(X_p)+\overline{A}_{p\cdot g}$$
	\end{proof}
	...
	\begin{proof}
		...
		$$T=\ell_{g^{-1}_{ji}(x)}\circ dg_{ji,x}(X_x)$$
	...
	\begin{Prop}\label{Prop_7.2.3}
		Let $F\in \Omega^2_Ad(P,\mathfrak{g})$ be a curvature form on a principal bundle and $F_M\in\Omega^2(M,E)$ be the corresponding $2$-form on the associated bundle. The transformation $\phi\in\mathcal{G}(P)$ on $F_M$ is induced by the one of $F$ and is:
	...
	\begin{proof}
		...
		But this is exactly $\phi^{-1}$ on $E$ (see proposition \ref{Prop_7.1.3}). Thus, we get:
		$$F_M\rightarrow \phi^{-1}\cdot F_M$$
	\end{proof}
	\section{Products on associated bundles}
	In this section we will define some operations on forms that take values on the associated bundle of a principal bundle. Those will be useful to define the full Y-M Lagrangian. More information on this topic can be found in [3] chap. 7 pag. 409-413.
	\begin{Def}
	...
	\begin{Ex}
		...
		Moreover, the Cartan-Killing form, from proposition \ref{Prop_2.8.2}, is invariant under Lie algebra automorphisms. In particular, it is $Ad$-invariant. This immediately implies by proposition \ref{Prop_7.1.3} that the scalar product induced in the adjoint bundle $Ad(P)$ is invariant under gauge transformations, since they transform with the adjoint representation.
	\end{Ex}
	\begin{Def}
	...
	\begin{proof}
		...
		$$\varphi: \pi_{Ad}^{-1}(U)\rightarrow U\times \mathfrak{g}\hbox{ such that }\phi([s(x)\cdot g,X])=(x,\rho(g)X)$$
	...
	In this chapter we will use the knowledge exposed previously to construct the famous Yang-Mills Lagrangian and look at some of its properties. More information on this topic can be found in REFERENZA
	\section{The Maxwell's equations}
	In this section we will see a geometric formulation for Maxwell's equations. We will first of all analyze them in a void flat space-time configuration. More references can be found in REFERENZA\\
	\\
	...
	In this section we will generalize the results of the previous analysis in a curved space-time. We will assume a background non-flat metric, with Minkowski signature. We will also keep the assumption of a void space-time. REFERENZA
	\\\\
	...
	$$\hbox{det}(g^{ij}_{\hspace{9pt}i\in I,j\in J})=\sum_\sigma g^{i_1j_{\sigma_1}}...g^{i_kj_{\sigma_k}}[\sigma_1...\sigma_k]$$
	where $\sigma$ is a permutation of the indices. Substituting this into the above expression:
	$$\star dx^I=\sqrt{|\hbox{det}g|}\sum_{J,\sigma} [JJ']g^{i_1j_{\sigma_1}}...g^{i_kj_{\sigma_k}}[\sigma_1,...,\sigma_k]dx^{J'}$$
	\begin{Prop}
	...
	\begin{proof}
		...
		In the last expansion $\braket{e_i(p),e_j(p)}_{Ad}=\braket{e_i(p),e_j(p)}_{\mathfrak{u}(1)}$ is a non degenerate, positive definite symmetric product on $\mathbb{R}=\mathfrak{u}(1)$. Since the group is abelian, the product is clearly invariant under the adjoint representation. Locally, we can choose an orthonormal frame $e_i$ for this product. Let now $\omega$ be the volume form, then $\braket{F_M^i,F_M^j}\omega=F_M^i\wedge\star F_M^j$ and in an orthonormal basis the Cartan-Killing form becomes: $$B_{\mathfrak{u}(1)}(e_i,e_j)=-\delta_{ij}$$
		at any $p$. Thus, we can write:
		...
		Choosing a local chart $(U,x^i)$ on $M$, we can select a basis $\{\partial_\mu\}$ for the tangent space and a base $\{dx^\mu\}$ for the cotangent space. Those bases are constructed to be orthonormal with respect to the Minkowski metric. It is now possible to expand with those coordinates the pullback of the curvature:
		...
		This is exactly the same form of proposition \ref{Prop_8.1.2}. Now it only remains to show that the solution of this Lagrangian satisfies Maxwell's equations. \\\\
	...
	In this section we will generalize the construction of the electromagnetic Lagrangian with higher dimensional groups. This will lead us to the QCD gauge boson Lagrangian. More information on this can be found in REFERENZA\\
	\\
	...
	In this section, we will construct a concrete example of the Yang-Mills theory on the Hopf bundle. More references on this topic can be found in REFERENZA
	\\\\
	...
	The projection is:
	$$\pi(cos(\theta)e^{i\phi},sin(\theta)e^{i\psi})=(2sin(\theta)cos(\theta)e^{i(\phi-\psi)},cos^2(\theta)-sin^2(\theta))=$$
	$$=(sin(2\theta)e^{i(\phi-\psi)},cos(2\theta))$$ 
	So, each point in $S^3$ is identified with a unique $(\theta,\phi,\psi)$. Now we wish to construct a connection on this principal bundle. By definition, since the Lie algebra of $U(1)$ is $\mathbb{R}$, this will be an element of $\Omega^1(S^3,\mathbb{R})$ which satisfies the properties of \ref{Def_6.6}. A generic $1$-form will be:
	...
	$$A(\overline{X})=X$$
	Where $\overline{X}$ is the fundamental vector field of $X$. Now it is only  a matter to compute the fundamental vector field of a generic element of the Lie algebra $i\alpha$. We have, by definition:
	...
	Given the projection $\pi$, we have a natural choice: since any point on $S^2$ is identifies with two angles $\alpha\in[0,\pi[$ and $\beta\in[0,2\pi[$
	...
	where $a$ is a smooth function on the manifold. Thus, there will exist a local section $\rho: U\subset S^2\rightarrow S^3$ such that:
	...
	Lastly, we wish to find a solution to Maxwell's equations. As usual, the first half $DF=0$ is trivially satisfied by construction. The remaining conditions, $D^*F_{S^2}=0$, are imposed on the form $F_{S^2}\in\Omega^2(S^2, Ad(S^3))$. Since the adjoint representation is trivial, $F_{S^2}$ coincides with the pullback of $F$ on $S^2$ defined above. To define the codifferential, one needs a metric on $S^2$. The standard choice is:
	$$g=\begin{pmatrix}
		1 && 0\\
		0 && sin^2(\alpha)
	\end{pmatrix}$$
	...
	$$F_{S^2}=-ksin(\alpha)d\alpha\wedge d\beta$$
	Lastly we can reduce the degrees of freedom of this form by using the gauge symmetry. In fact, in the $U(1)$ case, we have that the form $A$ is gauge equivalent to any $A'=A+d\Lambda$; where $\Lambda$ is a function of $(\alpha,\beta)$. We can choose as a gauge condition the Lorentz gauge:
	$$\partial^\mu A'_\mu=0$$ 
	...
	$$d\Lambda=-k\beta sin(\alpha)d\alpha+kcos(\alpha)d\beta$$
	This yields:
	$$A\rightarrow A'=kcos(\alpha)d\beta$$
	And again, the curvature is still:
	$$dA'=-ksin(\alpha)d\alpha\wedge d\beta$$

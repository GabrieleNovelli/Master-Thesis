...
\begin{document}
	...
	\begin{proof}
		...
		The last property follows immediatly from the fact that for real valued forms the pullback commutes with the exterior derivative.
	\end{proof}
	\begin{Ex}
	...
	In this chapter we will introduce the notion of connection on principal bundles, together with some properties of them. Those will be the main object of Gauge field theories. More informations can be found in [2] chap. 6 pag. 254-286; [3] chap. 4, 5.
		...
		\begin{proof}
			...
			\begin{itemize}
				...
				\item\textit{case 3}: both vectors are vertical. By properties of the connectio $1$-form:
	...
	\end{Ex}
	\begin{Theo}(\textbf{Musical isomorphism})
	...
	\end{proof}
	The previous result tells us that after choosing a local gauge, we can express every bundle automorphism as a $G$-valued map on the manifold.\\
	...
	\begin{Theo}
		...
		$$A_i=Ad(g_{ij}^{-1})A_j+\mu_{ij}$$
		Where $g_{ji}$ is the transition function between the local trivializations $s_i,s_j$ while the form $\mu_{ij}=g_{ij}^*\theta$ is the pullback of $\theta$ the Maurer-Cartan form.
	\end{Theo}
	\begin{proof}
		By construction, $A_i=s_i^*A$ so that for any vector field $X\in \Gamma(U)$, we have:
		$$s^*A(X)=A(ds(X))$$
		The two sections induce trivializations by PROPOSIZIONE. Furthermore, by OSSERVAZIONE, we know that the relation between the two sections at any point $x\in U$ is:

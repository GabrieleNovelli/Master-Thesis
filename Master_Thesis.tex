...
\begin{document}
	...
	\begin{Ex}
		...
		\\
		To find more informations on this example the reader can see [1] 105-107, 125, 165 chap. 3 and 4.\\
		\\
	...
	\begin{proof}
		...
		This is clearly equal to $\braket{X_g,Y_g}=\braket{\ell_{g^{-1}}(X_g),d\ell_{g^{-1}}(Y_g)}$ if and only if the action of the adjoint leaves the product unchanged.
	\end{proof}
	...
	\begin{proof}
		...
		In local coordinates, expanding $X=X^i\partial_i;X^*=X^*_jdx^j$, the components of $X^*_p$ will be given by the lowering action of the metric:
	...
	In this section we will introduce the notion of horizontal lift of a vector field and look at some properties of it. More information on this topic can be found in [2] chap. 6 pag 259, 260; [3] chap. 5 pag 286-289.\\
	\\
	...
	\begin{Def}
		Let $(P,M,\pi,G)$ be a principal $G$-bundle and $\rho:G\rightarrow GL(V)$ a representation on a finite dimensioinformationnal vector space $V$. We define the \textit{associated bundle} as the set: 
	...
	So, each point in $S^3$ is identified with a unique $(\theta,\phi,\psi)$. Now we wish to cosntruct a connection on this principal bundle. By definition, since the Lie algebra of $U(1)$ is $\mathbb{R}$, this will be an element of $\Omega^1(S^3,\mathbb{R})$ which satisfies the properties of \ref{Def_6.6}. A generic $1$-form will be:
	$$A=A^\theta d\theta+A^\phi d\phi+A^\psi d\psi$$
	Where $d\theta,d\phi,d\psi$ is the dual base of the coordinates: $(\partial_\theta,\partial_\phi,\partial_\psi)$. By \ref{Def_6.6}, this form must be smooth, this implies that the maps $A^\theta, A^\phi,A^\psi$ must be smooth.
	...
	$$r^*_gA=A$$
	The action of the group $U(1)$ is the following:
	$$e^{-i\alpha}(\theta,\phi,\psi)=(\theta,\phi-\alpha,\psi-\alpha)$$
	Imposing this we get:
	$$r^*_{e^{-i\alpha}}A_{\theta,\phi-\alpha,\psi-\alpha}(a_{(\theta,\phi,\psi)}\partial_\theta+b_{(\theta,\phi,\psi)}\partial_\phi+c_{(\theta,\phi,\psi)}\partial_\psi)=$$
	$$=A_{\theta,\phi-\alpha,\psi-\alpha}(dr_{e^{-i\alpha}}(a_{(\theta,\phi,\psi)}\partial_\theta+b_{(\theta,\phi,\psi)}\partial_\phi+c_{(\theta,\phi,\psi)}\partial_\psi))=$$
	$$=A^\theta a_{(\theta,\phi,\psi)}+A^{(\phi-\alpha)} b_{(\theta,\phi,\psi)}+A^{(\psi-\alpha)} c_{(\theta,\phi,\psi)}=$$
	$$=A^\theta a_{(\theta,\phi,\psi)}+A^{\phi} b_{(\theta,\phi,\psi)}+A^{\psi} c_{(\theta,\phi,\psi)}$$
	This implies that the components of $A$ do not depend on $\psi$ and $\phi$.\\\\
	Lastly, we impose that, for any $X$ element of the Li algebra, we have:
	$$A(\overline{X})=X$$
	Where $\overline{X}$ is the fundamental vector field of $X$. Now it is only  amatter to compute the fundamental vector field of a generic element of the Lie algebra $i\alpha$. We have, by definition:

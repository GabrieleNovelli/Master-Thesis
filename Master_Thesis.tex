...
\begin{document}
	...
	\begin{Def}
		...
		\end{itemize}
		We also refer to any manifold $M$ endowed with a metric with the name \textit{metric manifold}.
	\end{Def}
	...
	\end{Def}
		\begin{Obs}
		Once we choose a chart $(U,\phi)$ on $M$, with coordinates $\{x^i\}$, we know that we have an induced base of $T_pM$ of vectors $\{\partial_i|_p\}$ for any $p\in U$. We can then induce another base on the dual space $T_pM^*$, indexed with $\{dx^i_p\}$ defined by:
		$$dx^i_p(\partial_j|_p)=\delta^i_j$$
	\end{Obs}
	...
	\begin{Obs}
		...
		$$dx^i_p(\partial_j|_p)=\partial_j|_px^i=\delta^i_j$$
		...
		$$df_p(\partial_j|_p)=a_i(p)\delta^i_j=\partial_j|_pf$$
	...
	\end{Def}
	\begin{Obs}
		Since every vector space has only 2 orientations, and at each point the tangent space is indeed a vector space, any orientable manifold has only 2 possible orientations.
	\end{Obs}
	\begin{Ex}
		NASTRO DI MOBIUS??? NON ORIENTABILE
	\end{Ex}
	VOLUME FORM
	\section{Differential calculus on smooth manifolds}
	...
	\begin{proof}
		...
		$$=da_I\wedge dx^I\wedge b_Jdx^J+a_Idb_J\wedge dx^I\wedge b_Jdx^J=d\omega\wedge\beta+(-)^{deg\omega}\omega\wedge db_j\wedge\beta$$

...
\begin{document}
	...
	In 1919, Hermann Weyl proposed the idea of \textit{local scale or gauge invariance} in an attempt to unify electromagnetism and gravity. He introduced the term gauge invariance to describe a rescaling of the metric tensor. Although Weyl's choice was physically incorrect due to its wrong prediction of frequency shifts in atomic spectra, the term \textit{gauge} persisted and evolved in meaning over time, coming to signify local symmetry transformations rather than scale transformations.
	...
	\begin{Prop}\label{Prop_7.2.3}
		Let $F\in \Omega^2_{Ad}(P,\mathfrak{g})$ be a curvature form on a principal bundle and $F_M\in\Omega^2(M,E)$ be the corresponding $2$-form on the associated bundle. The transformation $\phi\in\mathcal{G}(P)$ on $F_M$ is induced by the one of $F$ and is:
	...
	\begin{proof}
		...
		$$[p,\hbox{Ad}(g^{-1})F_p(\tilde{X}_p,\tilde{Y}_p)]\sim [p\cdot \sigma^{-1}_\phi(p),F_p(\tilde{X}_p,\tilde{Y}_p)]=[\phi^{-1}(p),F_p(\tilde{X}_p,\tilde{Y}_p)]$$
	...
	\begin{Ex}
		...
		Moreover, the Cartan-Killing form, from proposition \ref{Prop_2.8.2}, is invariant under Lie algebra automorphisms. In particular, it is $Ad$-invariant. This immediately implies by proposition \ref{Prop_7.1.3} that the scalar product induced in the adjoint bundle $\hbox{Ad}(P)$ is invariant under gauge transformations, since they transform with the adjoint representation.
	\end{Ex}
	\begin{Def}
		Let $(P,M,\pi,G)$ a principal bundle, $\rho:G\rightarrow GL(V)$ a representation of $G$ and $E$ the induced associated bundle $E=P\times_\rho V$. Let $\omega_{1,2}\in \Omega^k_(M,E)$ be two $E$-valued forms on $M$. Then we define an inner product as follows: choose a local frame $\{e_i:M\rightarrow E\}$ of $E$ and expand $\omega_{1,2}=\omega_{1,2}^i\otimes e_i$, and set:
	...
	\begin{Prop}\label{God_Prop}
		Let $(P,M,\pi,G)$ be a principal bundle and $(\hbox{Ad}(P),M,\pi_{Ad},\mathfrak{g})$ be the adjoint associated bundle. Let $A\in \Omega^1(P,\mathfrak{g})$ be a connection on the principal bundle and $F\in\Omega^2_{Ad}(P,\mathfrak{g})$ be its curvature. Then, for any local gauge $s:U\subset M\rightarrow P$w, the pullback $s^*F$ of $F$ and the associated form $F_M\in\Omega^2(M,\hbox{Ad}(P))$ coincide.  
	\end{Prop}
	\begin{proof}
	...
	In this section we will generalize the results of the previous analysis in a curved space-time. We will assume a background non-flat metric, with Minkowski signature. We will also keep the assumption of a void space-time. [12] chap. 1 pag. 118-122.
	\\\\
	...
	In this section we will explicitly construct the Lagrangian of the electromagnetic field. More references on this topic can be found in [12] chap. 1 pag. 118-122, chap. 2 pag. 361-366.
	\\\\
	The Lagrangian is the mathematical object that allows us to describe the laws of the physical system. The aim is to construct a Lorentz-invariant Lagrangian, from which we can recover our equations: $dF=d^*F=0$. For the following analysis, we will assume that spacetime is empty and flat, i.e. the metric tensor is $\eta$ the Minkowski metric. 
	...
	In this section we will use the Maxwell's equation to deduce the general form of the Yang-Mills Lagrangian for the $U(1)$ case, i.e. electromagnetism. More references on this topic can be found in: [3] chap. 7 pag. 401-423.
	\\\\
	...
	\begin{Prop}
		...
		where $F_M$ is the induced curvature on $\Omega^2(M,\hbox{Ad}(P))$, $\hbox{Ad}(P)$ is the associated adjoint bundle constructed from the principal bundle $P$ and $\braket{,}_{Ad}$ is the bundle metric induced from the Cartan-Killing form on $U(1)$.
	\end{Prop}
	\begin{proof}

...
\begin{document}
	...
	\begin{proof}
		...
		So that $\hbox{ad}_{\sigma X}=\sigma\circ \hbox{ad}\circ \sigma^{-1}$. The immediatly implies:
	...
	In this section we define the concept of associated bundle and look at some properties of it, together with some examples. More informations on this topic can be found in REFERENZA
	\begin{Def}
	...
	\begin{Ex}\label{Ex_6.3.1}
		...
		$F=dA+{1\over 2}[A,A]$ and was proved to be equivariant and horizontal in \ref{Theo_5.5.1}. Moreover, the same result told us:
	...
	\begin{Obs}\label{Obs_6.3.5}
		...
		And a priori only the first term is 0. bhowever, one can prove that $DF=0$, where $F$ is the curvature of a connection $A$. Since $F$ is an $Ad$-tensorial $2$-form we can apply the formula shown in proposition \ref{Prop_6.3.4} in the associated adjoint bundle and get:
		$$DF=dF+[A,F]$$
	...
	\end{Def}
	Clearly, by linearity, this does not depend on the choice of the base.\\
	\\
	...
	\begin{proof}
		...
		In the last expansion $\braket{e_i(p),e_j(p)}_{Ad}=B_{\mathfrak{u}(1)}(e_i(p),e_j(p))$ is induced by the Killing form, which is a non degenerate, negative definitem symmetric product in our setting. This implies that locally we can choose an orthonormal frame $e_i$ for this produc. Let now $\omega$ be the volume form, then $\braket{F_M^i,F_M^j}\omega=F_M^i\wedge\star F_M^j$ and in our base the Killinig form takes the form: $B_{\mathfrak{u}(1)}(e_i,e_j)=-\delta_{ij}$ at any $p$. Thus, we can write:
